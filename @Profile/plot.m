...
% ---------------------------------------------------------------------
    ...
    function onMkSpectrum_1(~, ~)
        ...
        if execOpts.ExecutionMode == "parallel"
    ...
    function onMkSpectrum_2(~, ~)
        if ~obj.Interpolated
            error('Profile:plot:NotInterpolated', ...
    ...
    function showPeakPickingWindow(peakResult)
        %SHOWPEAKPICKINGWINDOW Display peak-picking diagnostics in 3 panels.
        %
        % Layout:
        % - top left  : original signal + baseline + accepted apexes
        % - top right : corrected signal + smoothed trace + peak limits
        % - bottom    : table of filtered results
        %
        % Input
        % -----
        % peakResult : structure returned by PeakPicking
        ...
        for iSeg = 1:numel(segments)
            ...
            % -----------------------------------------------------------------
            % Right panel: processed signal + limits
            % -----------------------------------------------------------------
...
if ~isfield(obj.AdditionalInformation, 'Finnee')
    error('Profile:plot:MissingFinnee', ...
...
if ~isfield(obj.AdditionalInformation, 'Dataset')
    error('Profile:plot:MissingDataset', ...
...
execOpts = struct('FastMode', false, 'ExecutionMode', "serial");
...
if isempty(answer) || strcmp(answer, 'Cancel')
    error('Profile:plot:UserCancelled', ...
